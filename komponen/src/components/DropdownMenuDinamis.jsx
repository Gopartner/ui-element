...
export default function DropdownMenuDinamis({ position = "right", wrapperRef }) {
  const [isOpen, setIsOpen] = useState(false);
  const [menuPosition, setMenuPosition] = useState(position); // Default posisi dropdown
  ...
  const handleOpenMenu = () => {
    if (buttonRef.current && wrapperRef?.current) {
      ...
      if (buttonRect.right + 200 > wrapperRect.right || buttonRect.right + 200 > viewportWidth) {
        // Jika tombol terlalu dekat tepi kanan
        setMenuPosition("right");
      } else if (buttonRect.left - 200 < wrapperRect.left || buttonRect.left - 200 < 0) {
        // Jika tombol terlalu dekat tepi kiri
        setMenuPosition("left");
      } else {
        setMenuPosition(position); // Default posisi
      }
    }
    setIsOpen((prev) => !prev);
  };
